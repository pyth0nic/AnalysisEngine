...
class MySpider(CrawlSpider):
    ...
    def parse(self, response):
        ...
        if "text" not in str(response.headers["Content-Type"]):
            ...
            next_links = [link.url for link in link_extractor.extract_links(response) if
                          not self.is_extracted(link.url)]
    ...
    def is_extracted(self, url):
        ...
        if clean_url not in self.crawled_ids:
            self.crawled_ids.add(url)
            return False
        return True

class FileLinkCrawler:
